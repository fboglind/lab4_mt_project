...
def extract_references(
    parallel_file, test_ids_file, output_file, src_col, tgt_col, logger
):
    ...
    try:
        ...
        logger.info(
            f"Found references for {found_count} out of {len(test_sentences)} test sentences"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Extract reference translations from parallel corpus"
    )
    parser.add_argument(
        "--parallel", required=True, help="Path to the parallel corpus TSV file"
    )
    parser.add_argument(
        "--test", required=True, help="Path to the file containing test sentences"
    )
    parser.add_argument(
        "--output", required=True, help="Path to save reference translations"
    )
    parser.add_argument(
        "--src-col",
        default="Russian",
    ...
    success = extract_references(
        args.parallel, args.test, args.output, args.src_col, args.tgt_col, logger
    )
