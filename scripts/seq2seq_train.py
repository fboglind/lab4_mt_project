...
class DataLoader:
    ...
    def build_vocabularies(self):
        """Build source and target vocabularies from the dataset"""
        ...
        for i, word in enumerate(set(" ".join(self.df[self.src_lang_col]).split())):
            if word not in self.src_vocab:
                self.src_vocab[word] = len(self.src_vocab)
        
        for i, word in enumerate(set(" ".join(self.df[self.tgt_lang_col]).split())):
        ...
        print(f"Vocabulary sizes: Source = {len(self.src_vocab)}, Target = {len(self.tgt_vocab)}")
    ...
    def get_batches(self, max_length=512):
        ...
        for i in range(0, len(data), self.batch_size):
            batch = data.iloc[i:min(i + self.batch_size, len(data))]  # Handle last batch
            ...
            for _, row in batch.iterrows():
                src_tensor = tensor_from_sentence(
                    self.src_vocab, row[self.src_lang_col], 
                ...
                tgt_tensor = tensor_from_sentence(
                    self.tgt_vocab, row[self.tgt_lang_col], 
                    pad_idx=self.special_tokens["PAD"], 
                    eos_idx=self.special_tokens["EOS"], 
                    unk_idx=self.special_tokens["UNK"],
                    max_length=max_length
...
class Trainer:
    ...
    def update_model(self, loss):
        ...
        try:
            ...
            torch.nn.utils.clip_grad_norm_(self.encoder.parameters(), max_norm=self.clip_value)
            torch.nn.utils.clip_grad_norm_(self.decoder.parameters(), max_norm=self.clip_value)
