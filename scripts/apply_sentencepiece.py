...
1. A **parallel corpus** (Russian-English), where:
   ...
   - If `--to_parallel` is specified, both Russian and English
     columns are tokenized.
2. A **test set** (Russian only), producing tokenized input for inference.

Usage Examples:
- Tokenize only Russian in a parallel corpus:
    python3 apply_sentencepiece.py --input data/parallel_corpus.tsv
    --output data/spm_parallel_corpus.tsv --model models/spm_ru.model
...
- Tokenize a test set for inference:
    python3 apply_sentencepiece.py --input data/test_raw_ru.txt
      --output data/test_preprocessed_ru.txt --model models/spm_ru.model
        --is_test_set
"""
...
parser = argparse.ArgumentParser(
    description="Apply SentencePiece tokenization to text data."
)
parser.add_argument(
    "--input", required=True, help="Input file (raw text or parallel corpus)"
)
...
parser.add_argument(
    "--to_parallel",
    action="store_true",
    help="Specify if output should be a parallel model",
)
parser.add_argument(
    "--is_test_set", action="store_true", help="Specify if input is a test set"
)
...
if not args.is_test_set:
    ...
    if "Russian" not in df.columns:
        raise ValueError(
            "Error: Column 'Russian' not found in input file."
            " Check if input is a valid parallel corpus."
        )
    ...
    if args.to_parallel:
        ...
        df["English"] = df["English"].apply(
            lambda x: " ".join(sp.encode(x, out_type=str))
        )

    # Save tokenized corpus
    df.to_csv(args.output, sep="\t", index=False, encoding="utf-8")
    print(
        f"SentencePiece tokenization complete. Processed {len(df)}\
              sentences. Saved to {args.output}"
    )
    ...
    print(
        f"SentencePiece tokenization complete. Processed {num_lines}\
              TEST sentences. Saved to {args.output}"
    )
