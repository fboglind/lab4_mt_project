...
def create_visualizations(bins, avg_bleu, avg_chrf, counts, output_dir):
    ...
    plt.savefig(
        os.path.join(output_dir, "bleu_vs_length.png"), dpi=300, bbox_inches="tight"
    )
    ...
    plt.savefig(
        os.path.join(output_dir, "chrf_vs_length.png"), dpi=300, bbox_inches="tight"
    )
...
def evaluate_translations(hyp_file, ref_files, output_dir, logger):
    ...
    if len(hypotheses) != min_len:
        logger.warning(
            f"Truncating hypotheses from {len(hypotheses)} to {min_len} sentences"
        )
    ...
    for i, refs in enumerate(references):
        if len(refs) != min_len:
            logger.warning(
                f"Truncating reference {i+1} from {len(refs)} to {min_len} sentences"
            )
    ...
    bins, avg_bleu, avg_chrf, counts = analyze_length_impact(
        hypotheses, references, logger
    )
    ...
    # Compile results
    evaluation_results = {
        "bleu": {"score": bleu_score.score, "details": str(bleu_score)},
        "chrf": {"score": chrf_score.score, "details": str(chrf_score)},
        "length_analysis": {
            "bins": bins,
            "avg_bleu": avg_bleu,
            "avg_chrf": avg_chrf,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Advanced machine translation evaluation"
    )
    parser.add_argument("--hyp", required=True, help="Path to hypothesis translations")
    parser.add_argument(
        "--ref", required=True, nargs="+", help="Path(s) to reference translation(s)"
    )
    parser.add_argument(
        "--output-dir",
