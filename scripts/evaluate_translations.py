...
def main():
    ...
    parser = argparse.ArgumentParser(description="Evaluate machine translation using sacreBLEU")
    parser.add_argument(
        "--hyp", 
        required=True, 
        help="Path to hypothesis translations (system output)"
    )
    parser.add_argument(
        "--ref", 
        required=True, 
        help="Path to reference translations (ground truth)"
    )
    parser.add_argument(
        "--output", 
